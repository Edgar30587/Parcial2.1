...
<head>
    ...
    <link rel="stylesheet" href="css/director_dashboard1.css">
</head>
<body>
    <div class="container">
        <div class="sidebar">
            <div class="welcome-message">
                <h2>Bienvenido</h2>
                <img src="img/avatar.png" alt="Avatar Director" class="avatar">
        ...
        <div class="main-content">
            <c:if test="${param.action == 'calificar_anteproyecto'}">
                <sql:update dataSource="${parcial2}">
                    INSERT INTO CalificacionesDirector (anteproyecto_id, director_id, fecha_evaluacion, calificacion_director, comentarios_director)
                    VALUES (?, ?, CURDATE(), ?, ?)
                    ON DUPLICATE KEY UPDATE
                    calificacion_director = VALUES(calificacion_director),
                    comentarios_director = VALUES(comentarios_director),
                    fecha_evaluacion = CURDATE()
                    <sql:param value="${param.anteproyecto_id}" />
                    <sql:param value="${sessionScope.user.director_id}" />
                    <sql:param value="${param.calificacion_director}" />
                    <sql:param value="${param.comentarios_director}" />
                </sql:update>
                <p class="success-message">Calificación del director guardada exitosamente.</p>
                <script>
                    setTimeout(function() {
            ...
            <div id="anteproyectos" class="tab-content">
                ...
                <sql:query var="anteproyectos" dataSource="${parcial2}">
                    ...
                    LEFT JOIN CalificacionesDirector cd ON a.anteproyecto_id = cd.anteproyecto_id AND cd.director_id = ?
                    WHERE a.director_id = ?
                    <sql:param value="${sessionScope.user.director_id}" />
                    <sql:param value="${sessionScope.user.director_id}" />
                </sql:query>
                ...
                <table>
                    <tr>
                        ...
                        <th>Estado</th>
                        <th>Documento</th>
                        <th>Calificación del Director</th>
                        <th>Acciones</th>
                    </tr>
                    <c:forEach var="anteproyecto" items="${anteproyectos.rows}">
                        <tr>
                            ...
                            <td>
                                ...
                                <form action="director_dashboard.jsp" method="post" style="display: inline;">
            ...
            <div id="calificar-anteproyecto" class="tab-content">
                ...
                <form action="director_dashboard.jsp" method="post">
                    ...
                    <label for="calificacion_director">Calificación del Director:</label>
                    <input type="number" id="calificacion_director" name="calificacion_director" min="0" max="5" step="0.1" required>
                    <label for="comentarios_director">Comentarios del Director:</label>
                    <textarea id="comentarios_director" name="comentarios_director" rows="4"></textarea>
                    <button type="submit">Guardar Calificación del Director</button>
                </form>
            </div>
